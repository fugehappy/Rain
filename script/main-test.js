  ...
  function startGameInit() {
    ...
    states.main = function (game) {
      ...
      this.startGame = function () {
        ...
        //TODO:
        let self = this;
        let data = new Date().valueOf() + time * 1000;
        $(".timeBar")
          .countdown(data.toString())
          .on("update.countdown", function (event) {
            if (self.leftTimeText) {
              self.leftTimeText.destroy();
            }
            date = event.strftime("%S");
            self.leftTimeText = game.add.text(40, 40, date, {
              fill: "#FFF",
              fontSize: "40px",
              fontWeight: "bolder",
            });
            self.leftTimeText.scale.setTo(0.8);
            console.log(date);
          })
      ...
      //Web Worker 异步 Web Workers 是 HTML5 提供的一个javascript多线程解决方案
      //New Date
      //时间差
